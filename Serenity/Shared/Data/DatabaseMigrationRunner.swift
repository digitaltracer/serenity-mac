...
actor DatabaseMigrationRunner {
  ...
  private static let migrations: [DatabaseMigration] = [
    ...
    DatabaseMigration(
      identifier: "20260919_012_custom_provider",
      statements: [
        // SQLite cannot alter a CHECK constraint, so each provider-constrained table is rebuilt.
        """
        CREATE TABLE ai_insights_new (
          id TEXT PRIMARY KEY,
          provider TEXT NOT NULL CHECK (provider IN ('openai', 'gemini', 'anthropic', 'nvidia', 'custom', 'local')),
          type TEXT NOT NULL CHECK (type IN ('productivity', 'behavior', 'recommendation', 'warning')),
          title TEXT NOT NULL,
          description TEXT NOT NULL,
          confidence REAL DEFAULT 0.5,
        ...
        CREATE TABLE ai_usage_new (
          id TEXT PRIMARY KEY,
          timestamp TEXT NOT NULL,
          provider TEXT NOT NULL CHECK (provider IN ('openai', 'gemini', 'anthropic', 'nvidia', 'custom')),
          operation TEXT NOT NULL CHECK (operation IN ('analyze', 'recap', 'quickadd', 'summary', 'slack')),
          prompt_tokens INTEGER DEFAULT 0,
          completion_tokens INTEGER DEFAULT 0,
          total_tokens INTEGER DEFAULT 0,
